...
HISTORY_LIMIT = 6          # last N messages loaded per request (3 turns)
SESSION_TTL_HOURS = 24     # sessions inactive longer than this get a fresh context


def get_or_create_session(session_id: str | None, bot: str, db: Session) -> str:
...
def load_history(session_id: str, db: Session, limit: int = HISTORY_LIMIT) -> list[dict]:
    """Return the last `limit` messages for a session, oldest first.

    Returns an empty list if the session has been inactive for more than SESSION_TTL_HOURS.
    """
...
def get_last_intent(session_id: str, db: Session) -> str | None:
