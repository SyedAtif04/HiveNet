            ...
            <tbody>
              {filtered.map((row, i) => (
                <tr key={row.id} className={`border-t border-fb-border/50 hover:bg-fb-card2/30 transition-colors ${i % 2 === 0 ? '' : 'bg-white/[0.01]'}`}>
                  ...
                  <td className={`px-4 py-3 text-sm font-bold font-mono ${row.amount > 0 ? 'text-fb-green' : 'text-fb-red'}`}>
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={5} className="px-4 py-12 text-center text-fb-muted text-sm">No transactions match the filter.</td></tr>
              )}
            </tbody>
          </table>
        </div>
        <div className="px-4 py-3 border-t border-fb-border text-xs text-fb-muted">
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Add Transaction">
        <div className="space-y-4">
              ...
              {['Income', 'Expense'].map(t => (
                <button key={t} onClick={() => setForm(f => ({ ...f, type: t }))}
                  className={`flex-1 py-2 rounded-lg text-xs font-semibold border transition-colors
                    ${form.type === t ? (t === 'Income' ? 'bg-fb-green-dim border-fb-green text-fb-green' : 'bg-fb-red-dim border-fb-red text-fb-red') : 'border-fb-border text-fb-muted hover:border-fb-dim'}`}>
                  {t}
                </button>
              ))}
            </div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button onClick={handleAdd} className="flex-1 py-2.5 rounded-lg btn-primary text-sm">Save Transaction</button>
          </div>
        </div>
      </Modal>
    </div>
