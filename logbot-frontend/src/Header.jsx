...
export default function Header({ page, onNavigate, alertCount = 7 }) {
  ...
  return (
    <header className="fixed top-0 left-56 right-0 h-14 bg-fb-sidebar/80 backdrop-blur-md border-b border-fb-border z-30 flex items-center px-6 gap-4">
      ...
      <button onClick={() => onNavigate('alerts')} className="relative w-8 h-8 rounded-lg bg-fb-card2 border border-fb-border flex items-center justify-center text-fb-muted hover:text-fb-accent hover:border-fb-accent/40 transition-colors">
        <Icons.Bell size={15} />
        {alertCount > 0 && (
          <span className="absolute -top-0.5 -right-0.5 w-3.5 h-3.5 bg-fb-red rounded-full text-[9px] font-bold text-white flex items-center justify-center">{alertCount}</span>
        )}
      </button>
