...
export const Card = ({ children, className = '', style = {}, onClick, onDragOver, onDragLeave, onDrop }) => (
  <div className={`card-glass rounded-xl border border-fb-border ${className}`} style={style} onClick={onClick} onDragOver={onDragOver} onDragLeave={onDragLeave} onDrop={onDrop}>
    {children}
  </div>
);
...
export const ActionCard = ({ label, sub, icon, onClick, active }) => (
  <button onClick={onClick}
    ...
    <div className={`w-9 h-9 rounded-lg flex items-center justify-center ${active ? 'bg-black/10' : 'bg-fb-card2'}`}>{icon}</div>
    <div>
...
export const PriorityBadge = ({ level }) => {
  ...
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-medium border uppercase ${cls[level] || cls.low}`}>
      {level}
    </span>
  );
};
...
export const PredCard = ({ period, data }) => (
  <Card className="p-5">
    <div className="text-xs font-semibold text-fb-muted uppercase tracking-wider mb-4">{period}</div>
    <div className="space-y-3">
      {[
        { label: 'Income',  val: data.income,  trend: data.incomeTrend,  up: true,  color: 'text-fb-green' },
        { label: 'Expense', val: data.expense, trend: data.expenseTrend, up: false, color: 'text-fb-red'   },
        { label: 'Profit',  val: data.profit,  trend: data.profitTrend,  up: true,  color: 'text-fb-blue'  },
      ].map(r => (
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1 text-xs font-medium text-fb-green">
          <Icons.TrendUp size={12} /> {data.trend}
        </div>
