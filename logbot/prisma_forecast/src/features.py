...
LAG_PERIODS = [1, 3, 5, 7, 14, 30]  # Enhanced with lag_3 and lag_5
...
def create_calendar_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Cyclic encoding for seasonality (sine/cosine transformations)
    ...
    print(f"   ✓ Cyclic seasonality features: month_sin/cos, week_sin/cos, day_of_week_sin/cos")
...
def create_change_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    Create change-based features (differences) for numeric columns
    ...
    print(f"   Creating change-based features (differences)...")

    created_features = []

    # Quantity change (diff)
    if TARGET_COLUMN in df.columns:
        feature_name = f'{TARGET_COLUMN}_diff'
        if feature_name not in df.columns:
            df[feature_name] = df[TARGET_COLUMN].diff()
            created_features.append(feature_name)

    # Price change (if price column exists)
    price_col = None
    for col in ['unit_price', 'price', 'cost', 'total_cost']:
        if col in df.columns:
            price_col = col
            break

    if price_col:
        feature_name = f'{price_col}_diff'
        if feature_name not in df.columns:
            df[feature_name] = df[price_col].diff()
            created_features.append(feature_name)

    # Economic indicator change (if exists)
    if 'economic_indicator' in df.columns:
        feature_name = 'economic_indicator_diff'
        if feature_name not in df.columns:
            df[feature_name] = df['economic_indicator'].diff()
            created_features.append(feature_name)

    # Market price index change (if exists)
    if 'market_price_index' in df.columns:
        feature_name = 'market_price_index_diff'
        if feature_name not in df.columns:
            df[feature_name] = df['market_price_index'].diff()
            created_features.append(feature_name)

    if created_features:
        print(f"   ✓ Created {len(created_features)} change-based features: {', '.join(created_features)}")
    else:
        print(f"   ✓ Using existing change-based features")
